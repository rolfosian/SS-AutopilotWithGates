...
import static data.scripts.autopilotwithgates.util.UiUtil.*;
...
public class AbilityScroller {
    ...
    public AbilityScroller(UIPanelAPI abilityPanel) {
        ...
        final ButtonAPI prevButton = (ButtonAPI) abilityPanelPrevButtonHandle.get(abilityPanel);
        final ButtonAPI nextButton = (ButtonAPI) abilityPanelNextButtonHandle.get(abilityPanel);
                ...
                for (InputEventAPI event : events) {
                    if (!event.isConsumed() && event.isMouseScrollEvent() && isInBounds(event.getX(), event.getY())) {
                        if (event.getEventValue() > 0) {
                            utils.actionPerformed(abilityPanel, null, prevButton);
                        } else {
                            utils.actionPerformed(abilityPanel, null, nextButton);
                        }
                        event.consume();
                    }
                }
        ...
        this.abilityPanel.addComponent((UIComponentAPI)this.abilityBarIdxLabel).rightOfMid((UIComponentAPI)nextButton, 6f).setYAlignOffset(-2f);

        this.oldAbilitySlots = CampaignEngine.getInstance().getUIData().getAbilitySlots();
        this.ourAbilitySlots = new AbilitySlotters();

        setAbilitySlots(this.ourAbilitySlots, this.oldAbilitySlots.getSlots());
